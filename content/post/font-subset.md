...
용량이 과도하게 크면 네트워크 환경이나 단말기 성능이 낮은 사용자는 긴 로딩 시간으로 불편을 겪을 수 있기 때문이다.

이번 포스팅에서는 폰트 용량을 효과적으로 줄이는 방법인 **‘서브셋’** 개념을 소개하고, 이를 활용해 폰트를 직접 커스터마이징을 해보려고 한다.
...
※ 개인적인 의견이지만, 만약 임의로 폰트를 서브셋하여 사용하는 경우라면, **소속 프로젝트 내에서만 제한적으로 사용하고 외부 재배포는 삼가는 것이 바람직하다.**  
