...
리액트를 처음 접하실 때 가장 첫번째로 맞닥드리는 통곡의 벽은 아마 `Context API`가 아닐까 합니다. 사실 컨텍스트 자체에 대한 개념을 이해하지 못한 상태에서 바로 실무에 적용하기에는 난잡하고 복잡한 부분이 많지요.
그래도 사용하다보면 어느 정도 두려움이 사라지고 재미있고 즐겁게 사용할 수 있게 됩니다.
이 글에서는 `Context API`에 대한 막연한 어려움을 해소함과 동시에, 타입스크립트와 함께 연동해서 실사용에 적합한 코드 구조를 짜 동료분들의 실무에 약간의 도움이 되는 시간을 가져볼까 합니다.

## Context란?

Context(이하 컨텍스트)는 언어학에서 주로 쓰이는 말입니다. 언어는 텍스트와 컨텍스트의 집합체다라는 말이 있지요. 텍스트는 단순히 글자들만을 나타냅니다. 그러한 텍스트들이 어떤 상황이나 상태를 표현하는 문맥과 합쳐저 진정한 언어가 됩니다. 즉 컨텍스트의 본질은 **환경**이라고 할 수 있겠습니다.

프로그래밍에서도 컨텍스트가 가지는 본질은 동일합니다. 다만 세부내용이 약간은 다릅니다. 컴퓨터과학에서의 컨텍스트는 **수행되는 작업이나 프로세스가 실행되는 환경** 이라고 해석할 수 있겠습니다. 이번에 우리가 다룰 Context API에서의 컨텍스트는 함수나 메서드, 변수를 참조할 수 있는 하나의 공간(문맥)을 의미한다고 생각할 수 있겠네요!

실제로 리액트의 Context API는 위의 정의와 동일한 역할을 수행합니다. **컨텍스트 API는 컴포넌트 간 깊은 Props Drilling이 생기는 것을 막기 위해 존재하는 기능이지만, 그 기능은 컨텍스트라는 정의를 통해 구현되었습니다.**

컨텍스트 API를 사용하면 개발자가 변수를 담을 문맥을 생성하여 특정 컴포넌트에 전역적으로 사용할 수 있게 합니다. 이를 통해 상태 관리가 더 간편해지고, 컴포넌트 간의 데이터 전달이 효율적으로 이뤄지지요.
...
일반적으로, 부모 컴포넌트에서 자식 컴포넌트로 정보를 넘길때는 props를 이용합니다. 하지만 컴포넌트의 길이가 매우 깊어지면 props 관리가 매우 힘들어지지요. 위 사진처럼 컴포넌트가 너무 줄줄이 고구마처럼 엮여있으면 유지보수가 험난해질 것은 너무나도 자명한 사실입니다. 이런 순간이 닥쳐올 것을 막기 위해 컨텍스트 API가 등장했습니다. **컨텍스트를 이용하면 컨텍스트 제공자(Context Provider)로 감싸진 하위 트리에 속하는 컴포넌트들에 대해 컨텍스트의 정보를 가져다 사용하게 함으로써 props drilling을 효과적으로 없애버릴 수 있습니다.**
...
0. 컨텍스트를 사용하려면 컨텍스트를 만들어야합니다. 리액트에서 제공하는 createContext 함수를 통해 생성할 수 있지요.
1. 0번에서 만들어진 컨텍스트를 사용하려면 provider를 통해 컨텍스트에 추가할 값을 전달해야 합니다.
2. 또한 provider에 감싸져있는 컴포넌트 트리들은 자신을 감싼 context를 사용할 수 있게 됩니다.

글로만 보았을 땐 개념 이해가 어려웠는데 그림으로 그려보니 직관적으로 다가오는 것 같습니다.

## Context API 사용 with 타입스크립트

그럼 마지막으로 해당 컨텍스트를 사용하는 소스코드를 공유하고 물러나도록 하겠습니다. 아래 코드는 제가 Context API를 사용할 때 주로 사용하는 구조입니다.
